...
public class IncomingMessageTask implements ActiveTask {
    ...
    public void execute() throws Exception {
        ...
        if (repositoryDAO.retrieve(repositoryDAOData)) {
            ...
            AS2PlusProcessor.getInstance().getIncomingMessageProcessor().processReceivedMessage(incomingMessage);
        }
        else {
    ...
    public void onFailure(Throwable e) {
        AS2PlusProcessor.getInstance().getLogger().error("Incoming message task failure", e);
        if (retried == getMaxRetries()) {
            try {
                ...
                AS2PlusProcessor.getInstance().getLogger().error("Unable to mark failure to incoming AS2 message: "+message.getMessageId(), ex);
            }
        }
    }
