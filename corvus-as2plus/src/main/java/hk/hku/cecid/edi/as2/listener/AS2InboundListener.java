    ...
    public void processRequest(AS2Request request, AS2Response response) 
        ...
        try {
            ...
            if (requestMessage.isDispositionNotification()) {
                AS2PlusProcessor.getInstance().getIncomingMessageProcessor().processReceipt(requestMessage);
            }
            else {
                AS2Message responseMessage = AS2PlusProcessor.getInstance().getIncomingMessageProcessor().processMessage(requestMessage);
