...
public class OutgoingMessageTask implements ActiveTask {
    ...
    public void execute() throws Exception {
        ...
        if (repositoryDAO.retrieve(repositoryDVO)) {
            ...
            try {
                ...
                AS2Processor.core.log.info("Sending outgoing "+outgoingMessage+" to "+url);
                ...
                try {
                    ...
                    AS2Processor.core.log.error(outgoingMessage + " was sent but exception occurred in receiving receipt", t);
                }
            }
            catch (Exception e) {
            ...
            throw new AS2Exception("No repository found for outgoing AS2 message: "+message.getMessageId());
        }
    }
    ...
    public void onFailure(Throwable e) {
        AS2Processor.core.log.error("Outgoing message task failure", e);
        if (!isRetryEnabled() || retried == getMaxRetries()) {
            try {
                ...
                AS2Processor.core.log.error("Unable to mark failure to outgoing AS2 message: "+message.getMessageId(), ex);
            }
        }
    }
